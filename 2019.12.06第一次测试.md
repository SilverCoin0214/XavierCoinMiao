...
5. 什么是 GUI，什么是 CLI，什么是接口/界面？现实生活中有哪些例子？
   ...
   - 现实生活中浏览器显示就是界面, 而浏览器背后连接的服务器就是通过接口来进行数据传输.
...
26. 为什么不同类型的标签的 fallback 内容要以不同的形式提供？

    -

27. 分别写出在 head 中设定页面编码，设定 icon，引入样式表的标签
...
29. 写出以下几个符号的 ascii 码：`a，A，0，CR，LF，空格，NBSP`。
...
43. 如何让页面打印时应用不同的效果？
    ...
    - 也可以单独设置CSS文件, 设置Link里的media="print"属性来指定样式专门用于打印
